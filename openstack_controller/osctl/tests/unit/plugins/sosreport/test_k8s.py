...
from openstack_controller.osctl.plugins.sosreport import k8s
...
class TestK8sObjectsCollector(utils.BaseTestCase):
    def setUp(self):
        ...
        mock_node_objects = mock.patch(
            "openstack_controller.kube.Node.objects"
        )
        self.mock_node_obj = mock_node_objects.start()
        self.addCleanup(mock_node_objects.stop)

        self.mock_node = mock.patch("openstack_controller.kube.Node")
