...
if check_env_var("MYSQL_MARIABACKUP_USERNAME"):
    mysql_mariabackup_username = os.environ['MYSQL_MARIABACKUP_USERNAME']
if check_env_var("MYSQL_MARIABACKUP_PASSWORD"):
    mysql_mariabackup_password = os.environ['MYSQL_MARIABACKUP_PASSWORD']
...
if mysql_dbadmin_username == mysql_mariabackup_username:
    logger.critical(
        "The dbadmin username should not match the backup admin username")
    sys.exit(1)
...
def mysqld_write_cluster_conf(mode='run'):
    ...
    cluster_config_params['wsrep_sst_auth'] = "{0}:{1}".format(
        ...
        if not mysql_dbaudit_username:
            template = (
                ...
                "SHUTDOWN ;".format(mysql_dbadmin_username, mysql_dbadmin_password,
                                    mysql_dbsst_username, mysql_dbsst_password,
                                    mysql_mariabackup_username, mysql_mariabackup_password))
        else:
            template = (
                "DELETE FROM mysql.user WHERE user != 'mariadb.sys' ;\n"
                "CREATE OR REPLACE USER '{0}'@'%' IDENTIFIED BY \'{1}\' ;\n"
...
def check_for_active_nodes(endpoints_name=direct_svc_name,
    ...
    if active_endpoints and len(active_endpoints) >= 1:
        ...
        # NOTE(vsaienko): pause here and let active endpoint do its job if it was just
        # started to avoid PRODX-36113
        time.sleep(30)
        return True
    else:
...
def run_mysqld(cluster='existing'):
    ...
    if os.path.isdir(db_test_dir):
        ...
        # next sleep has been added to prevent Galera cluster crash in case when instance was in the cluster but lost content
        # of /var/lib/mysql/ dir and than container restart quickly and the instance try to reconnect to the exist cluster
        # BUG - PRODX-26032
        time.sleep(60)
        logger.info(
