...
{{- end }}
    ...
    {{- if hasKey $envAll.Values "conf" }}
      {{- if hasKey $envAll.Values.conf "dependencies" }}
        {{- if hasKey $envAll.Values.conf.dependencies "delay" }}
    - name: DEPENDENCY_DELAY
      value: {{ toJson $envAll.Values.conf.dependencies.delay | quote }}
        {{- end }}
      {{- end }}
    {{- end }}
  command:
