...
{{- if .Values.conf.convert_to_uuid.compute.image_ref_raw }}
{{- $imageRefRaw := .Values.conf.convert_to_uuid.compute.image_ref_raw }}
# Get image ref raw id
IMAGE_REF_RAW=$(openstack image show {{ $imageRefRaw }} -f value -c id)

# Check if image ref raw not set
die_if_not_set $LINENO IMAGE_REF_RAW "Failure getting IMAGE_REF_RAW for {{ $imageRefRaw }}"

# Set image ref raw id to tempest configuration file
iniset $TEMPEST_CONF compute image_ref_raw $IMAGE_REF_RAW
{{- end }}
